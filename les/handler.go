...
import (
	...
	"github.com/kodabs/go-kodabsdiuma/core/state"
	...
	"github.com/kodabs/go-kodabsdiuma/light"
	...
	"github.com/kodabs/go-kodabsdiuma/trie"
)
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Kodabsdium protocol")
	...
	log.Info("Light Kodabsdium protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light Kodabsdium peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light Kodabsdium handshake failed", "err", err)
	...
	if err := pm.peers.Register(p); err != nil {
		clientErrorMeter.Mark(1)
		p.Log().Error("Light Kodabsdium peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Kodabsdium message handling failed", "err", err)
			if p.fcServer != nil {
				p.fcServer.DumpLogs()
			}
			return err
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Kodabsdium message arrived", "code", msg.Code, "bytes", msg.Size)
