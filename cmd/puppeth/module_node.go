...
{{end}}
RUN \
  echo 'gkds --cache 512 init /genesis.json' > gkds.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.kodabsdium/keystore/ && cp /signer.json /root/.kodabsdium/keystore/' >> gkds.sh && \{{end}}
	echo $'exec gkds --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gkds.sh

ENTRYPOINT ["/bin/sh", "gkds.sh"]
`
...
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.kodabsdium{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.kdshash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.kodabsdium"],
		ethashdir:  infos.volumes["/root/.kdshash"],
		port:       port,
